...
st.set_page_config(page_title="DKidsFantasias · Previsão", page_icon="🔮", layout="wide")
ui.inject_css()
...
ui.hero(
    ...
    f"Projeção dos próximos 30 dias de venda com XGBoost · Histórico: {periodo_dados(base)}",
)
...
ui.section(
    ...
    "Roxo: faturamento real · Rosa: previsão do modelo · Faixa: intervalo de confiança de 95% na projeção",
)

hist_plot = hist.copy()
hist_plot["tipo"] = hist_plot["conjunto"].map({"Treino": "Real", "Validação": "Real (validação)"})
...
with c1:
    ...
    barras_sem = alt.Chart(sem).mark_bar(cornerRadiusTopLeft=6, cornerRadiusTopRight=6, color=PRIMARY, size=40).encode(
