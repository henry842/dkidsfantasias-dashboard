...
e sem dependências. É só abrir o [`index.html`](index.html) desta pasta e pronto.
...
1. Abra `index.html` (o arquivo desta pasta) no navegador (duplo clique).
...
A pasta [`../docs/`](../docs) é o espelho de publicação: contém uma cópia desta
pasta (`webapp/`) + `data/vendas_tratadas.csv` pronta para o GitHub Pages servir
direto do branch `main`.
...
Envie esta pasta (`webapp/`) + `../data/` para qualquer hospedagem estática
(Netlify, Vercel, S3...). A aplicação encontra o CSV em `../data/` automaticamente.
...
Depois de atualizar os dados ou editar esta pasta, rode `../sync_docs.sh` (a
partir da raiz do repositório) para espelhar as mudanças em `../docs/` antes de
dar commit/push — é isso que mantém o GitHub Pages em dia.

## 🗂️ Arquitetura desta pasta
...
No repositório, esta pasta convive com:
- `../data/vendas_tratadas.csv` — base tratada de vendas
- `../docs/` — espelho de publicação (GitHub Pages serve daqui)
- `../sync_docs.sh` — copia esta pasta + `../data/` para `../docs/`
- `../export.py` — pipeline que gera o CSV tratado a partir do bruto
